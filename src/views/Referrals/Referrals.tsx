...
const Referrals: React.FC = () => {
    ...
    async function fetchMyAPI() {
      if (account) {
        const refcommison = await gettotalReferralCommissions(account);
         await getReferralCount(account);
  ...
  return (
    <div>
      <Hero>
        <CardBody>
          <Heading as="h1" size="xl" mb="22px" color="#ffffff">
            Cryptospace Referral Program
          </Heading>
        ...
        {account ? (
          <>
            ...
            <ReferralLink>
              ...
              <Line />
              <LinkReferral color="#ffffff" href={link} >{link}</LinkReferral>
            </ReferralLink>
          </>
        ) : (
          <ReferralWalletCard>
